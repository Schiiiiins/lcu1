...
struct ser_req {
	__be16			sample;
/*
 * We insert padding to the end of the cache line here to avoid cache 
 * line sharing issues when driver is used with DMA-based SPI controllers
 * (like atmel_spi) on systems where main memory is not DMA-coherent
 * (most non-x86 boards).
 */
    char        __padalign[L1_CACHE_BYTES - sizeof(__be16)];
	u8			ref_on;
	u8			command;
	u8			ref_off;
	u16			scratch;
