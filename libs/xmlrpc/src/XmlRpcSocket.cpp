...
bool 
XmlRpcSocket::bind(int fd, const char* path)
{
#if defined(_WINDOWS)
...
bool
XmlRpcSocket::connect(int fd, const std::string& path)
{
#if defined(_WINDOWS)
	return false;//unix domain sockets are not supported on windows
#else
    ...
    if (n > 0) {
      ...
      *eof = true;
      ...
      wouldBlock = true;
        ...
        XmlRpcUtil::log(5, "XmlRpcSocket::nbRead: error %d", XmlRpcSocket::getError());
      return false;   // Error
    }
  }
  return true;
  ...
  while ( nToWrite > 0 && ! wouldBlock ) {
    ...
    if (n > 0) {
      ...
      *bytesSoFar += n;
      ...
      XmlRpcUtil::log(5, "XmlRpcSocket::nbWrite: error %d", XmlRpcSocket::getError());
      return false;   // Error
    }
  }
  return true;
...
int XmlRpcSocket::getSocketPort(int fd)
{
