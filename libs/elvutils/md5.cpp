...
/*static*/ std::string md5::MD5String(const char* szString)
{
	...
	alg.Update((const uchar*)szString, (unsigned int)nLen);
	alg.Finalize();
...
// returning the results as a string.  Returns empty string if it fails.
...
void md5::Transform (const uchar* block)
{
...
// a multiple of 4.
void md5::Decode(uint4* dest, const uchar* src, uint4 nLength)
{
	uint4 i, j;
