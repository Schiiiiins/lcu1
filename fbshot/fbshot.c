...
int read_fb(const char *device, int vt_num, struct picture *pict){
  int fd, vt_old, i,j;
  ...
  if(!(pict->buffer=(char*)malloc(pict->xres*pict->yres*i)))
...
void convertBGRtoRGB(struct picture *pict){
  int i;
  for( i=0; i<pict->xres*pict->yres*3; i+=3 )
  {
      __u8 c = pict->buffer[i];
      pict->buffer[i] = pict->buffer[i+2];
      pict->buffer[i+2] = c;
  }
}
  ...
  switch(pict->bps) {
    case 8:
      ...
      for (i=0; i<(pict->yres); i++)
        row_pointers[i]=(png_byte*)pict->buffer+i*4*(pict->xres);
    ...
    case 16:
      ...
      for (i=0; i<(pict->yres); i++)
        row_pointers[i]=(png_byte*)pict->buffer+i*3*(pict->xres);
      png_set_invert_alpha(png_ptr);
      break;

    case 24:
      bit_depth=8;
      convertBGRtoRGB(pict);
      color_type=PNG_COLOR_TYPE_RGB;
      for (i=0; i<(pict->yres); i++)
        row_pointers[i]=(png_byte*)pict->buffer+i*3*(pict->xres);
      png_set_invert_alpha(png_ptr);
...
int main(int argc, char **argv){
  char *buffer, *outfile = argv[argc-1];
